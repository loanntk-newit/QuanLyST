...
<head>
    ...
    <title>CT Phiếu Nhập | Quản Lý Siêu Thị</title>
...
<body class="bg-light">
        ...
        <div class="container-fluid container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto ">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active text-sm-start text-center" aria-current="page" href="/ct-phieu-nhap">CT Phiếu Nhập</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container mt-5">
        ...
        <div class="card mb-5">
            ...
            <!-- CT Phiếu Nhập data -->
            <div class="card-body">
                <h3 class="card-title">CT Phiếu Nhập</h3>
                ...
                <?php
                $pn = mysqli_query($db, "SELECT * FROM `PHIEUNHAP`");
                $hh = mysqli_query($db, "SELECT * FROM `HANGHOA`");
                ?>


                <form class="row g-3" action="create.php" method="POST">
                    <div class="col-md-6">
                        <label for="MaPN" class="form-label">Phiếu Nhập</label>
                        <select class="form-select" name="MaPN" aria-label="Default select example">
                            <?php
                            while ($item = mysqli_fetch_array($pn)) {
                                echo $item;
                                echo "<option value='" . $item['MaPN'] . "'>" . $item['MaPN'] . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        <label for="MaHH" class="form-label">Hàng Hóa</label>
                        <select class="form-select" name="MaHH" aria-label="Default select example">
                            <?php
                            while ($item = mysqli_fetch_array($hh)) {
                                echo $item;
                                echo "<option value='" . $item['MaHH'] . "'>" . $item['TenHangHoa'] . "</option>";
                            }
                            ?>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        <label for="SLuongNhap" class="form-label">Số Lượng Nhập</label>
                        <input type="number" step=0.01 name="SLuongNhap" class="form-control" placeholder="10" required>
                    </div>

                    <div class="col-md-6">
                        <label for="GiaNhap" class="form-label">Giá Nhập</label>
                        <input type="number" step=0.01 name="GiaNhap" class="form-control" placeholder="10000" required>
                    </div>
        ...
        <div class="table-responsive mb-5 card">
            ...
            echo "<th scope='col'>Mã Phiếu Nhập</th>";
            echo "<th scope='col'>Mã Hàng Hóa</th>";
            echo "<th scope='col'>Số Lượng Nhập</th>";
            echo "<th scope='col'>Giá Nhập</th>";
            ...
            $data = mysqli_query($db, "SELECT * FROM CTPHIEUNHAP");
            $amount_data = mysqli_num_rows($data);
            $total_pages = ceil($amount_data / $per_page);

            $data_mhs = mysqli_query($db, "SELECT * FROM CTPHIEUNHAP LIMIT $page, $per_page");
            $no = $page + 1;
            ...
            while ($data = mysqli_fetch_array($data_mhs)) {
                echo "<tr>";
                echo "<td style='display:none'>" . $data['MaPN'] . "</td>";
                echo "<td class='text-center'>" . $no++ . "</td>";
                echo "<td>" . $data['MaPN'] . "</td>";
                echo "<td>" . $data['MaHH'] . "</td>";
                echo "<td>" . $data['SLuongNhap'] . "</td>";
                echo "<td>" . $data['GiaNhap'] . "</td>";
                    ...
                    <form action='edit.php' method='POST'>
                        <div class='modal-body text-start'>
                            ...
                            <div class="col-12 mb-3">
                                <label for="edit_MaPN" class="form-label">Mã Phiếu Nhập</label>
                                <input type="text" name="edit_MaPN" id="edit_MaPN" class="form-control" placeholder="MPN01" readonly>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="edit_MaHH" class="form-label">Mã Hàng Hóa</label>
                                <input type="text" name="edit_MaHH" id="edit_MaHH" class="form-control" placeholder="MHH01" readonly>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="edit_SLuongNhap" class="form-label">Số Lượng Nhập</label>
                                <input type="number" step=0.01 name="edit_SLuongNhap" id="edit_SLuongNhap" class="form-control" placeholder="10" required>
                            </div>

                            <div class="col-12 mb-3">
                                <label for="edit_GiaNhap" class="form-label">Giá Nhập</label>
                                <input type="number" step=0.01 name="edit_GiaNhap" id="edit_GiaNhap" class="form-control" placeholder="100000" required>
                            </div>
                        </div>
            ...
            <div class='modal-dialog'>
                <div class='modal-content'>
                    ...
                    <form action='delete.php' method='POST'>
                        ...
                        <div class='modal-body text-start'>
                            <input type='hidden' name='delete_MaPN' id='delete_MaPN'>
                            <input type='hidden' name='delete_MaHH' id='delete_MaHH'>
    ...
    <script>
        $(document).ready(function() {
            $('.editButton').on('click', function() {
                ...
                console.log(data);
                $('#edit_MaPN').val(data[0]);
                $('#edit_MaHH').val(data[3]);
                $('#edit_SLuongNhap').val(data[4]);
                $('#edit_GiaNhap').val(data[5]);
            });
        });
    </script>
    ...
    <script>
        $(document).ready(function() {
            $('.deleteButton').on('click', function() {
                ...
                console.log(data);
                $('#delete_MaPN').val(data[0]);
                $('#delete_MaHH').val(data[3]);
            });
        });
    </script>
