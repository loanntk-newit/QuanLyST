...
<head>
    ...
    <title>Phiếu Nhập | Quản Lý Siêu Thị</title>
        ...
        <div class="container-fluid container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto ">
                    ...
                    <li class="nav-item">
                        <a class="nav-link active text-sm-start text-center" aria-current="page" href="/phieu-nhap">Phiếu Nhập</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container mt-5">
        ...
        <div class="card mb-5">
            ...
            <div class="card-body">
                ...
                <?php
                $ncc = mysqli_query($db, "SELECT * FROM `NHACUNGCAP`");
                ...
                <form class="row g-3" action="create.php" method="POST">
                    ...
                    <div class="col-md-4">
                        <label for="MaPN" class="form-label">Mã Phiếu Nhập</label>
                        <input type="text" name="MaPN" class="form-control" placeholder="MPN01" required>
                    </div>

                    <div class="col-md-4">
                        <label for="MaNCC" class="form-label">Nhà Cung Cấp</label>
                        <select class="form-select" name="MaNCC" aria-label="Default select example">
                            <?php
                            while ($item = mysqli_fetch_array($ncc)) {
                                echo $item;
                    ...
                    <div class="col-md-6">
                        <label for="TongTienNhap" class="form-label">Tổng Tiền Nhập</label>
                        <input type="number" step=0.01 name="TongTienNhap" class="form-control" placeholder="1000000" required>
                    </div>

                    <div class="col-md-6">
                        <label for="NgayNhap" class="form-label">Ngày Nhập</label>
                        <input type="date" step=0.01 name="NgayNhap" class="form-control" required>
                    </div>
        ...
        <div class="table-responsive mb-5 card">
            ...
            echo "<th scope='col'>Mã Nhà Cung Cấp</th>";
            echo "<th scope='col'>Mã Nhân Viên</th>";
            echo "<th scope='col'>Tổng Tiền Nhập</th>";
            echo "<th scope='col'>Ngày Nhập</th>";
            ...
            $data = mysqli_query($db, "SELECT * FROM PHIEUNHAP");
            $amount_data = mysqli_num_rows($data);
            $total_pages = ceil($amount_data / $per_page);

            $data_mhs = mysqli_query($db, "SELECT * FROM PHIEUNHAP LIMIT $page, $per_page");
            $no = $page + 1;
            ...
            while ($data = mysqli_fetch_array($data_mhs)) {
                ...
                echo "<td>" . $data['TongTienNhap'] . "</td>";
                echo "<td>" . $data['NgayNhap'] . "</td>";
                ...
                <div class='modal-content'>
                    ...
                    <form action='edit.php' method='POST'>
                        <div class='modal-body text-start'>
                            ...
                            <div class="col-12 mb-3">
                                <label for="edit_MaNCC" class="form-label">Nhà Cung Cấp</label>
                                <select class="form-select" name="edit_MaNCC" id="edit_MaNCC" aria-label="Default select example">
                                    <?php
                                    while ($item = mysqli_fetch_array($ncc)) {
                                        echo $item;
                                        echo "<option value='" . $item['MaNCC'] . "'>" . $item['TenNCC'] . "</option>";
                            ...
                            <div class="col-md-6">
                                <label for="edit_TongTienNhap" class="form-label">Tổng Tiền Nhập</label>
                                <input type="number" step=0.01 name="edit_TongTienNhap" id="edit_TongTienNhap" class="form-control" placeholder="1000000" required>
                            </div>

                            <div class="col-md-6">
                                <label for="edit_NgayNhap" class="form-label">Ngày Nhập</label>
                                <input type="date" step=0.01 name="edit_NgayNhap" id="edit_NgayNhap" class="form-control" required>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $('.editButton').on('click', function() {
                ...
                $('#edit_MaNCC').val(data[3]);
                $('#edit_MaNV').val(data[4]);
                $('#edit_TongTienNhap').val(data[5]);
                $('#edit_NgayNhap').val(data[6]);
            });
        });
    </script>
